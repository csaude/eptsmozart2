...
where e.voided=0 and p.voided=0 and e.encounter_type in (5,7,80,81) and e.encounter_datetime<=:endDate and e.location_id IN (:locations)

union

select    pg.patient_id
from      sourceDatabase.patient p inner join sourceDatabase.patient_program pg on p.patient_id=pg.patient_id
where   pg.voided=0 and p.voided=0 and program_id in (1,2,25) and date_enrolled<=:endDate and location_id IN (:locations)
