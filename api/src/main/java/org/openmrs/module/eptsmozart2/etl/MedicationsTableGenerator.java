...
public class MedicationsTableGenerator extends AbstractGenerator {
	...
	protected PreparedStatement prepareInsertStatement(ResultSet results, Integer batchSize) throws SQLException {
		...
		String insertSql = new StringBuilder("INSERT INTO ")
				...
				.append("arv_side_effects, arv_side_effects_concept, adherence, adherence_concept, source_database) VALUES ")
		        .append("(?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)").toString();
		...
		try (Connection connection = ConnectionPool.getConnection()) {
			...
			String fetchMoreObsSql = new StringBuilder("SELECT o.*, d.name as formulation, d.combination, cn.name as concept_name ")
					...
					.append("AND !cn.voided AND cn.locale='en' AND (cn.locale_preferred OR cn.concept_name_type = 'FULLY_SPECIFIED') ")
			...
			while (results.next() && count < batchSize) {
				...
				parameterCache.put(1, currentEncounterId);

				parameterCache.put(2, results.getString("encounter_uuid"));
				insertStatement.setString(2, results.getString("encounter_uuid"));

				parameterCache.put(3, results.getInt("patient_id"));
				insertStatement.setInt(3, results.getInt("patient_id"));

				parameterCache.put(4, results.getString("patient_uuid"));
				...
				parameterCache.put(7, results.getInt("value_coded"));
	...
	protected String fetchQuery(Integer start, Integer batchSize) {
		StringBuilder sb = new StringBuilder(
		        ...
		        .append(
		            "cn.locale_preferred OR cn.concept_name_type = 'FULLY_SPECIFIED') WHERE !o.voided AND o.concept_id IN ")
		        .append(inClause(REGIMEN_CONCEPT_IDS)).append(" ORDER BY o.obs_id");
