...
public class IdentifierTableGenerator extends AbstractGenerator {
	...
	protected String countQuery() {
		return new StringBuilder("SELECT COUNT(*) FROM ")
				...
				.append(AppProperties.getInstance().getNewDatabaseName()).append(".patient p ON e.patient_id = p.patient_id JOIN ")
				.append(AppProperties.getInstance().getDatabaseName())
				.append(".patient_identifier_type pit on pi.identifier_type = pit.patient_identifier_type_id AND !pit.retired ")
				.append("WHERE !pi.voided").toString();
	}
	
	@Override
	protected String fetchQuery(Integer start, Integer batchSize) {
		StringBuilder sb = new StringBuilder("SELECT pi.patient_id, p.patient_uuid, pi.identifier_type, pit.name, pi.identifier, ")
