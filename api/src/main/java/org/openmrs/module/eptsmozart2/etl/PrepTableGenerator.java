...
public class PrepTableGenerator extends AbstractScrollableResultSetGenerator {
	...
	protected void setInsertSqlParameters(Set<Integer> positionsNotSet) throws SQLException {
		...
		if (resultConceptId == 1040 || resultConceptId == 165194) {
			insertStatement.setInt(HIV_TEST_RES_POS, valueCoded);
			positionsNotSet.remove(HIV_TEST_RES_POS);
			Timestamp hivResDate = null;
			if (encounterType == 81) {
				hivResDate = scrollableResultSet.getTimestamp("obs_datetime");
			} else if (encounterType == 80) {
				hivResDate = scrollableResultSet.getTimestamp("value_datetime");
			}
			if (hivResDate != null) {
				insertStatement.setTimestamp(HIV_TEST_RES_DATE_POS, hivResDate);
		...
		} else if (resultConceptId == 165174 && encounterType == 81) {
		...
		} else if (resultConceptId == 165294 && encounterType == 81) {
			insertStatement.setInt(RENAL_FAILURE_POS, valueCoded);
			positionsNotSet.remove(RENAL_FAILURE_POS);
		} else if (resultConceptId == 1713 && encounterType == 81) {
			...
			positionsNotSet.remove(LACTANT_POS);
		} else if (resultConceptId == 165225 && encounterType == 81) {
			switch (valueCoded) {
				case 1169:
		...
		} else if (resultConceptId == 165228 && encounterType == 81) {
			insertStatement.setInt(NEXT_CONSULT_POS, valueCoded);
