...
public class ProphylaxisTableGenerator extends AbstractScrollableResultSetGenerator {
	...
	protected String countQuery() {
		return new StringBuilder("SELECT COUNT(DISTINCT e.encounter_id) FROM ")
		        ...
		        .append(" AND e.concept_id IN ").append(inClause(PROPHYLAXIS_CONCEPT_IDS)).toString();
	}
	...
	protected String fetchQuery() {
		StringBuilder sb = new StringBuilder("SELECT e.*, p.patient_uuid, l.uuid as loc_uuid FROM ")
		        ...
		        .append(inClause(ENCOUNTER_TYPE_IDS)).append(" AND e.location_id IN ")
		        .append(inClause(Mozart2Properties.getInstance().getLocationsIds().toArray(new Integer[0])))
		        .append(" AND e.concept_id IN ").append(inClause(PROPHYLAXIS_CONCEPT_IDS)).append(" JOIN ")
